    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      ...
      <div className="flex-1 flex flex-col justify-center mx-3">
        ...
        <Link to={`/artists/${song?.artists[0].adamid}`}>
          <p className="text-base font-semibold text-gray-300 mt-1">{song?.subtitle}</p>
        </Link>
      </div>
    </div>
    <PlayPause isPlaying={isPlaying} song={song} handlePause={handlePauseClick} handlePlay={handlePlayClick} activeSong={activeSong} />
  </div>
);
    ...
    <div className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col" ref={divRef}>
      <div className="w-full flex flex-col">
        <div className="flex flex-row justify-between items-center">
          ...
          <Link to="/top-charts">
            <p className="cursor-pointer text-gray-400 text-base">See More</p>
          </Link>
        </div>

        <div className="mt-4 flex flex-col gap-1">
          {topPlays?.map((song, i) => (
            <TopChartCard song={song} i={i} isPlaying={isPlaying} activeSong={activeSong} handlePlayClick={() => handlePlayClick(song, i)} handlePauseClick={handlePauseClick} key={song.key} />
          ))}
        </div>
      </div>
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        <Swiper slidesPerView="auto" spaceBetween={15} centeredSlides freeMode centeredSlidesBounds modules={[FreeMode]} className="mt-4">
          {topPlays?.map((song, i) => (
